...
import os
from utils import extract_paddle, save_files, justification, extract_format, search_resumes, set_upstream, get_openai_embedding
import json
import pandas as pd
...
sidebar_width = 150  # Adjust this value to make the sidebar smaller
...
if option == 'Resume Filter':
    ...
    st.markdown("""<div class="top-bar"><div>Chatbot</div></div>""", unsafe_allow_html=True)

    user_input = st.text_input("Ask a question:")
    if user_input:
        results = search_resumes(user_input)
        # Display results
        for match in results["matches"]:
            st.write(f"Resume ID: {match['id']}, Score: {match['score']}")
